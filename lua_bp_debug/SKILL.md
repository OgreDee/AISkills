...
```bash
# 部署到当前项目
.claude/skills/lua_bp_debug/install.sh

# 部署到指定项目
.claude/skills/lua_bp_debug/install.sh /path/to/unity/project
```
...
| 文件 | 部署位置 | 说明 |
|------|----------|------|
| `scripts/DebugHook.txt` | `Assets/HotRes/Lua/Claude/DebugHook.txt` | Lua 断点 Hook |
| `scripts/DebugCaptureWriter.cs` | `Assets/Claude/Editor/DebugCaptureWriter.cs` | JSONL 写入器 |
| `scripts/LuaDebugMcpTool.cs` | `Assets/Claude/Editor/LuaDebugMcpTool.cs` | MCP lua_debug 工具 |
...
- `Assets/HotRes/Lua/Claude/DebugHook.txt` 已存在（v2: crl 组合 hook）
- `Assets/Claude/Editor/DebugCaptureWriter.cs` 已存在
...
| # | 检查 | 方法 | 失败处理 |
|---|------|------|----------|
| 1 | `DebugHook.txt` 存在 | `Glob("**/Claude/DebugHook.txt")` | 提示用户：需要先部署 DebugHook，提供文件路径 |
| 2 | `DebugCaptureWriter.cs` 存在 | `Glob("**/Claude/Editor/DebugCaptureWriter.cs")` | 提示用户：需要先部署 C# 侧 JSONL 写入器 |
...
**单断点**（最常用）：
```lua
require("Claude/DebugHook").WatchLine("RPG/Entity/EntityHero", 120)
```

**多断点**（多个可疑位置）：
```lua
require("Claude/DebugHook").WatchMulti({
...
**Nil 检测**：
```lua
require("Claude/DebugHook").WatchNil("UI/Hero/UI_Hero_Main", 402, "heroBaseConf")
```
...
**函数入口**：
```lua
require("Claude/DebugHook").WatchCall("RPG/Entity/EntityHero", "TakeDamage")
```
...
{
  "code": "package.loaded['Claude/DebugHook']=nil; require('Claude/DebugHook').WatchMulti({...})"
}
```

注入前先用 `package.loaded['Claude/DebugHook']=nil` 清除缓存，确保加载最新版本。
